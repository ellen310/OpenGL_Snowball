...
GLfloat   fDelta = 0.0f;  GLboolean bBlend= GL_TRUE;  GLboolean bSmooth = GL_TRUE;

GLfloat Red = 0.0;  GLfloat Green = 0.0;  GLfloat Blue = 0.0;  GLint   Index = 0;
...
void main_keyboard(unsigned char key, int x, int y) {
    ...
    case 't':
        ...
        if (!bBlend)
            ...
            bBlend = bSmooth = GL_TRUE;	//두가지 함수 모두 사용

        else

            bBlend = bSmooth = GL_FALSE;	//두가지 함수 모두 사용안함
...
int main(int argc, char** argv) {
    ...
    glutTimerFunc(1000, world_timer, 1);
